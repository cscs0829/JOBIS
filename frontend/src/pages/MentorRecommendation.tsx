...
const MentorRecommendation = () => {
  ...
  const executeSearch = useCallback(async (criteria: MentorSearchCriteria) => {
    ...
    try {
      const query = [
        ...
        criteria.minYearsExperience ? `${criteria.minYearsExperience}년 이상 경력` : ""
      ].filter(Boolean).join(" ");

      console.log("🔥 최종 검색 쿼리:", query);

      const response = await axios.post("http://localhost:9000/mentor-recommendation", {
        query: query,
      });
  ...
  const handleApplySearch = useCallback((criteria: MentorSearchCriteria) => {
